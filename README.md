**Maze AI Game** — это простая игра с искусственным интеллектом, в которой агент учится находить путь в лабиринте, избегая стен и получая штрафы за каждый шаг. Обучение реализовано с помощью алгоритма Q-Learning.
...
При запуске игры лабиринт и агент визуализируются. Цветовые обозначения:
- **Белый** — путь.
- **Серый** — стены.
...
При каждом шаге агент обновляет значения в Q-таблице для выбора оптимального действия. Формула обновления Q-значения основана на разнице между ожидаемой наградой и текущим состоянием:
...
- \( \alpha \) — learning rate, который определяет степень обновления (чем выше значение, тем сильнее обновление),
- \( r \) — полученная награда за действие \( a \),
- \( \gamma \) — discount factor, отвечающий за значимость будущих наград,
- \( \max_{a'} Q(s', a') \) — максимальное значение Q для следующего состояния \( s' \), которое отражает лучшее возможное действие.

### TD Error (Ошибка временной разницы)

**TD Error** — это разница между целевым значением и текущим Q-значением:
